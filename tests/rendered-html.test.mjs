...
assert.match(ordersScreen, /order-rich-meta/);
  ...
  assert.match(ordersScreen, /Data do faturamento/);
  assert.match(ordersScreen, /order\.DTFAT/);
  ...
  assert.match(dataSource, /kind === "orders"[\s\S]*?F\.DTENTSAI >= \$\{startExpression\}/);
  assert.match(dataSource, /kind === "orders"[\s\S]*?AND C\.DTNEG >= \$\{startExpression\}/);
  assert.match(dataSource, /kind === "orders"[\s\S]*?C\.CODTIPOPER IN \(5, 6\)/);
  assert.match(syncSource, /C\.CODTIPOPER IN \(5, 6\)/);
  assert.match(syncSource, /DTFAT/);
...
test("restricts and renders the consolidated company sales monitor", async () => {
  ...
  assert.match(dataSource, /TRUNC\(C\.DTENTSAI, 'MM'\)/);
